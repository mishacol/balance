...
export const ExpenseTrendWidget: React.FC<ExpenseTrendWidgetProps> = ({ 
  ...
  // Chart data state
  const [chartData, setChartData] = useState<ExpenseTrendData[]>([]);
  const [dataKey, setDataKey] = useState('month');
  const [isLoading, setIsLoading] = useState(false);
  const [monthsWithRecords, setMonthsWithRecords] = useState(0);

  // Get available expense categories from transactions
  ...
  const generateChartData = async () => {
    ...
    try {
      ...
      // Filter expense transactions for the selected period and category
      ...
      // Group transactions by aggregation level and track categories
      const groupedData: Record<string, { total: number; categories: Record<string, number> }> = {};
      ...
      for (const transaction of periodExpenses) {
        ...
        if (!groupedData[groupKeyValue]) {
          groupedData[groupKeyValue] = { total: 0, categories: {} };
        }
        ...
        // Track category (only if showing general trend, not when filtering by category)
        if (!selectedCategory) {
          const category = transaction.category || 'other';
          if (!groupedData[groupKeyValue].categories[category]) {
            groupedData[groupKeyValue].categories[category] = 0;
          }
          groupedData[groupKeyValue].categories[category] += convertedAmount;
        }
      }
      ...
      console.error('Error generating expense trend chart:', error);
    } finally {
      setIsLoading(false);
    }
  };

  // Update chart when period or category changes
  useEffect(() => {
    generateChartData();
  }, [selectedPeriod, customStartDate, customEndDate, selectedCategory, transactions, baseCurrency]);
  ...
  const formatCategoryName = (category: string): string => {
    const categoryMap: { [key: string]: string } = {
      ...
      'charitable-subscriptions': 'Charitable Subscriptions',
      'donations': 'Donations',
      'charity': 'Charity',
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      ...
      const dataPoint = expensePayload?.payload;
      const categories = dataPoint?.categories as Record<string, number> | undefined;
      
      return (
        <div className="bg-gray-800 border border-gray-600 rounded-lg p-3 shadow-lg max-w-xs">
          <p className="text-white font-medium mb-2">{label}</p>
          {expensePayload && (
            <p className="text-expense font-bold text-lg mb-2">
              Total: {formatCurrency(expensePayload.value, baseCurrency)}
            </p>
          )}
                ...
                {Object.entries(categories)
                  ...
                  .map(([category, amount]) => (
                    <div key={category} className="flex justify-between items-center text-xs">
                      ...
                      <span className="text-expense font-semibold ml-2">
  ...
  // Calculate total expenses and period info
  const totalExpenses = chartData.reduce((sum, item) => sum + (item.expenses as number), 0);
  ...
  const monthlyAverage = monthsWithRecords > 0 ? totalExpenses / monthsWithRecords : 0;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-gradient-to-br from-surface to-background border border-expense/30 rounded-xl p-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <div className="text-gray-400 text-sm font-medium">Total Expenses</div>
              </div>
            ...
            {chartData.length > 0 ? (
              ...
              <div className="flex items-center justify-center h-64">
                <div className="text-center">
                  <div className="text-gray-400 text-sm">No expense data to display</div>
                </div>
              </div>
            )}
          </div>
